...
const currentLanguage =
	...
	window.location.hash.slice(1, 3) || window.navigator.language.slice(0, 2) || 'en';
...
function changeLanguage(lang) {
	...
	// 	if (langList[prop] && langList[prop][value]) {
	// 		item.textContent = langList[prop][value][lang];
	// 		if (item.getAttribute("placeholder")) {
	// 			item.placeholder = langList[prop][value][lang];
	// 		}
	// 	}
	// 	else {
	...
	// 	// item.textContent = langList[prop]?.[value]?.[lang];
	// 	// slice ktruma te qani hatn es uzum, orinak grum es 0-ic minchev qo nshac texy mer depqum hima index
	...
	const observer = new IntersectionObserver((entries) => {
		entries.forEach((entry) => {
			if (entry.isIntersecting) {
				fetch(`https://bright-renewal-4fd12368a6.strapiapp.com/api/${entry.target.dataset.language}?populate=*&locale=${lang}`)
					.then((res) => res.json())
					.then(({ data }) => {

						switch (entry.target.dataset.language) {


							case 'header':
								entry.target.querySelector('.nav__list').innerHTML = '';
								data.links.forEach((link) => {
									entry.target.querySelector('.nav__list').innerHTML += `<li class="nav__item" data-nav-item><a href="${link.link}" class="nav__link link title" data-lang="header-home">${link.text}</a></li> `;
								})
								break;

							case 'hero':
								entry.target.querySelector('.hero__name').innerHTML = data.name;
								entry.target.querySelector('.hero__lastname').innerHTML = data.lastname;
								entry.target.querySelector('.hero__descr').innerHTML = data.description;
								entry.target.querySelector('.hero__picture source').srcset = data.img_desktop.url;
								entry.target.querySelector('.hero__img').src = data.img_mobile.url;
								break;


							case 'about':
								entry.target.querySelector('.about__title').textContent = data.title;
								entry.target.querySelector('.about__descr').innerHTML = data.text;
								break;

							case 'skillhead':
								entry.target.querySelector('.skills__title').textContent = data.title;
								entry.target.querySelector('.skills__text').innerHTML = data.text;
								break;
								...
								data?.forEach((skill) => {
									entry.target.innerHTML += `
									 <li class="skills__items">
											<img class="skills__img" src="${skill.img.url}" alt="${skill.title}" />
											<h3 class="skills__items-title">${skill.title}</h3>
											<svg class="skills__rating" data-rating="${skill.rating}" width="168" height="21" viewBox="0 0 168 21" fill="none"
							...
							case 'portfoliohead':
								entry.target.querySelector('.portfolio__title').innerHTML = data.title;
								break;

							case 'contact':
								entry.target.querySelector('.contact__title').textContent = data.title;
								entry.target.querySelector('.contact__text').innerHTML = data.text;
								entry.target.querySelector('.contact__btn').innerHTML = data.buttontext;
								entry.target.querySelector('.footer__text2').innerHTML = data.copyright;
								break;

							case 'popup':
								entry.target.querySelector('.popup__title').innerHTML = data.title;
								entry.target.querySelector('.popup__input_name').placeholder = data.Name;
								entry.target.querySelector('.popup__input_email').placeholder = data.Email;
								entry.target.querySelector('.popup__input_phone').placeholder = data.Phone;
								entry.target.querySelector('.popup__input_message').placeholder = data.Message;
								entry.target.querySelector('.popup__input_send').innerHTML = data.Send;
								break;
...
buttonRu.forEach((button, index) => {
	...
	button.addEventListener("click", () => {
		buttonEn[index].classList.remove("language__button_active");
...
function rating() {
	document.querySelectorAll('[data-rating]')?.forEach(item => {
		...
		item.querySelectorAll('path').forEach((path, index) => {
			path.style.fill = index < starValue ? "#FFC107" : "#E5E5E5";
		})
